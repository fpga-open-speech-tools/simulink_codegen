def CreateProbeFunction(inputParams):
    ...
    functionString += "************************************************/\n"
    functionString += "static int " + inputParams.device_name + "_probe(struct platform_device *pdev) {\n"
    functionString += "  int ret_val = -EBUSY;\n"
    functionString += ("  char device_name[" + str(len(inputParams.device_name) + 12) +"] = \"fe_" + inputParams.device_name
    ...
    devpString = "fe_" + inputParams.device_name + "_devp"
    functionString += "  fe_" + inputParams.device_name + "_dev_t * " + devpString + ";\n"
    functionString += "  struct resource *r = NULL;\n"
    functionString += "  pr_info(\"" + inputParams.device_name + "_probe enter\\n\");\n"
    if inputParams.device_type == 2:  # FPGA device
        ...
        functionString += "  }\n"
    functionString += ("  " + devpString + " = devm_kzalloc(&pdev->dev, sizeof(fe_" + inputParams.device_name +
                       "_dev_t), GFP_KERNEL);\n")
    if inputParams.device_type == 2:  # FPGA device type
    ...
    functionString += "  status = alloc_chrdev_region(&dev_num, 0, 1, \"fe_" + inputParams.device_name + "_\");\n"
    ...
    functionString += "  cdev_init(&" + devpString + "->cdev, &fe_" + inputParams.device_name + "_fops);\n"
    functionString += "  status = cdev_add(&" + devpString + "->cdev, dev_num, 1);\n"
    functionString += "  if (status != 0)\n"
    functionString += "    goto bad_cdev_add;\n"
    functionString += "  device_obj = device_create(cl, NULL, dev_num, NULL, device_name);\n"
...
def CreateAttributeFilesAndErrorHandling(inputParams):
    ...
    for i in range(len(inputParams.device_attributes)):
        functionString += "  status = device_create_file(device_obj, &dev_attr_" + inputParams.device_attributes[i] + ");\n"
    ...
    functionString += "  if (status)\n"
    functionString += "    goto bad_device_create_file_" + str(len(inputParams.device_attributes) + 1) + ";\n"
    functionString += "  pr_info(\"HA_probe exit\\n\");\n"
    functionString += "  return 0;\n"
    functionString += "bad_device_create_file_" + str(len(inputParams.device_attributes) + 1) + ":\n"
    functionString += "  device_remove_file(device_obj, &dev_attr_name);\n"
    for i in range(len(inputParams.device_attributes) - 1, -1, -1):
        functionString += "bad_device_create_file_" + str(i) + ":\n"
        functionString += "  device_remove_file(device_obj, &dev_attr_" + inputParams.device_attributes[i] + ");\n\n"
    functionString += "bad_device_create:\n"
    if inputParams.device_type == 2:
        functionString += "  device_destroy(cl, dev_num);\n\n"
    functionString += "  cdev_del(&fe_" + inputParams.device_name + "_devp->cdev);\n"
    ...
    if inputParams.device_type == 2: # FPGA
        functionString += "bad_ioremap:\n"
        functionString += "  ret_val = PTR_ERR(fe_" + inputParams.device_name + "_devp->regs);\n"
        functionString += "bad_exit_return:\n"
        functionString += "  pr_info(\"" + inputParams.device_name + "_probe bad exit\\n\");\n"
