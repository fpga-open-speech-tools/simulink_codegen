...
class qsys_templates:
    ...
    def add_custom_component_connections(self, component_name, target):
        ...
        if(self.custom_components_added == 0):
            ...
            built_string += custom_component_initial_audio_in_connection_template.replace("component_name", component_name).replace("audio_in", target.audio_in)
        if((self.custom_components_added + 1) == self.num_custom_components):
            built_string += custom_component_final_audio_out_connection_template.replace("component_name", component_name).replace("audio_out", target.audio_out)
            self.custom_components_added = 0        
            return built_string
