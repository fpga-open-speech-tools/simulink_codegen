...
from .target import DE10, Arria10, Target
...
def create_connections(target, config, template):
    ...
    for custom_component in config.custom_components:
            built_string += template.add_custom_component_connections(custom_component, target)
    return built_string
...
def create_tcl_system_file(target, config, template, working_dir):
    ...
    with open(working_dir + tcl_file, "w") as out_file:
        out_file.write(f"package require -exact qsys {config.quartus_version}\n")
...
def run_cmd_and_log(cmd, log_msg, log_file_path, err_on_fail=True):
    ...
    if return_code != 0 and err_on_fail:
        raise ChildProcessError(f"The following command failed {cmd} \n The log file can be found at {log_file_path}")
...
def gen_qsys_file(tcl_file, working_dir):
    ...
    cmd = f'cd {working_dir} & ' + 'qsys-script ' + f'--script={tcl_file} ' + f'--search-path="../../../../../component_library/**/*,$" '
...
def gen_qsys_system(qsys_file, working_dir):
    ...
    working_dir : string
        Working directory of the generation process
    """
    cmd = f'cd {working_dir} & qsys-generate --synthesis=VHDL --search-path="../component_library/**/*,$"  {qsys_file}'
...
def gen_project_tcl(target, template, working_dir):
    ...
    with open(working_dir + "make_project.tcl", "w") as proj_file:
        proj_file.write(template.add_quartus_project(target.system_name, target))
    ...
    with open(RES_DIR + top_level_vhdl_file, "r") as orig_top_file:
        with open(working_dir + top_level_vhdl_file, "w") as new_top_file:
            for line in orig_top_file:
                new_top_file.write( line.replace( original_system, target.system_name ) )
...
def gen_pll_qsys(working_dir):
    ...
    cmd =  f'cd {working_dir} & qsys-generate --synthesis=VHDL --search-path="../component_library/**/*,$"  {pll_file}'
...
def gen_project(working_dir):
    ...
    cmd = f"cd {working_dir} & quartus_sh -t make_project.tcl"
...
def gen_rbf(working_dir, target_system):
    ...
    cmd = f"cd {working_dir}/output_files & quartus_cpf -c -m FPP {target_system}.sof {target_system}.rbf"
...
def execute_quartus_workflow(config, working_dir = ""):
    ...
    if(config.target_system == "de10"):
        ...
        raise ValueError(f"The provided target: {config.target_system} is not supported")
    ...
    template = qsys_templates(len(config.custom_components), int(target.base_address))
